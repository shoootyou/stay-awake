...
Non-Sleep is a lightweight tray utility that prevents your computer from going idle.
...
After launching Non-Sleep, you will see an icon in your system tray. The app does
...
Choose how Non-Sleep keeps your computer awake. See [Modes](#modes) below for details.
...
Non-Sleep supports four modes. Each mode keeps your computer from going idle, but
...
| **Autostart** | Launch Non-Sleep automatically when you log in. |
...
When a mouse-based mode is active, Non-Sleep checks if you have moved the cursor
...
3. Enable **Non-Sleep** in the list.
4. Return to Non-Sleep — the status will update to "Accessibility OK".
...
Non-Sleep stores its settings as a JSON file in your system's config directory:
...
**Does Non-Sleep appear in the Dock (macOS)?**
No. Non-Sleep is a menu-bar-only utility and does not show a Dock icon.

**Does Non-Sleep require admin or elevated privileges?**
